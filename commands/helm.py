        ...
        if (obj.db.shield["exist"]):
            for i in range(constants.MAX_SHIELD_NAME):
                ...
                if (not obj.db.shield[i]["active"] or not d):
                    buffer+= "|cRating:|n "+ "{:.15s}".format(unparse.unparse_power(obj.db.shield[i]["maximum"]/obj.db.shield[i]["ratio"])) + " " + "{:.15s}".format(unparse.unparse_power(obj.db.shield[i]["maximum"]))+ " "+ ":.15s".parse(unparse.unparse_power(obj.db.alloc["shield"][i]*obj.db.power["total"]))+"\n"
                else:
