        ...
        if (obj.db.cloak["exist"] or obj.db.shield["exist"]):
            ...
            if (obj.db.shield["exist"]):
                for i in range(constants.MAX_SHIELD_NAME):
                    ...
                    d = utils.sdb2dissipation(obj,i)
                    if (not obj.db.shield[i]["active"] or not d):
                        table.add_row("|cRating:|n",unparse.unparse_power(obj.db.shield["maximum"]/obj.db.shield["ratio"]),unparse.unparse_power(obj.db.shield["maximum"]),unparse.unparse_power(obj.db.alloc["shield"][i]*obj.db.power["total"]))
                    else:
