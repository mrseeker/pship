...
def do_border_report(self):
    ...
    if(errors.error_on_console(self.caller,obj)):
        ...
        for empire in search_tag(tag=constants.EMPIRE_ATTR_NAME,category="space_object"):
            if (empire is not None):
                range = xyz2range(obj.db.coords["x"],obj.db.coords["y"],obj.db.coords["z"],empire.db.coords["x"],empire.db.coords["y"],empire.db.coords["z"]) / constants.PARSEC
                if (math.fabs(range - empire.db.radius) >= 100.0):
                    continue
                buffer += f'{empire.name:>20}'
                if (range > empire.db.radius):
                    buffer += f'{int(xy2bearing((empire.db.coords["x"] - obj.db.coords["x"]),(empire.db.coords["y"] - obj.db.coords["y"]))):3d} {int(xyz2elevation((empire.db.coords["x"] - obj.db.coords["x"]),(empire.db.coords["y"] - obj.db.coords["y"]),(empire.db.coords["z"] - obj.db.coords["z"]))):>3d} {unparse.unparse_range((range - empire.db.radius) * PARSEC):>7s} Outside  '
                else:
                    buffer += f'{int(xy2bearing((obj.db.coords["x"] - empire.db.coords["x"]),(obj.db.coords["y"] - empire.db.coords["y"]))):3d} {int(xyz2elevation((obj.db.coords["x"] - empire.db.coords["x"]),(obj.db.coords["y"] - empire.db.coords["y"]),(obj.db.coords["z"] - empire.db.coords["z"]))):>3d} {unparse.unparse_range((empire.db.radius - range) * PARSEC):>7s} Inside   '
